...
def searchElement(the_list, item):
    if item in the_list:
        return True
def tester(max_limit, step):
    ...
    plt.ylabel('Time(ms)')
    plt.legend(['Searching at the start', 'Searching at the middle', 'Searching at the end'])
    plt.show()
max_limit = 1000000
step = max_limit // 100

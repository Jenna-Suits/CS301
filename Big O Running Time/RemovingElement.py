...
def removeElement(the_list, insertion):
    del the_list[insertion]
def tester(max_limit, step):
    ...
    plt.ylabel('Time(ms)')
    plt.legend(['Remove at the start', 'Remove at the middle', 'Remove at the end'])
    plt.show()
max_limit = 1000000
step = max_limit // 100
